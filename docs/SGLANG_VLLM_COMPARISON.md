...
> **참고 자료**
>
> 이 문서는 서빙 프레임워크 선택을 위한 개념 비교 자료이며, 현재 버전의 성능
> 벤치마크나 설치 호환성 계약이 아니다. Project NuriLab의 현재 구현 경로는
> `README.md`와 코드에 정의된 vLLM OpenAI-compatible API를 기준으로 한다.
> 실제 도입 판단 전에는 동일 모델, 동일 GPU, 동일 요청 데이터로 다시 측정한다.
