...
| 코드 리뷰와 병합 이력 | GitHub Pull Request |
...
| Linear 작업 이슈 작성 항목 | `docs/CONTRIBUTING.md` |
| Legacy GitHub Issue 형식 | `.github/ISSUE_TEMPLATE/task.md` |
...
- Linear `Nurilab` Issue는 작업 단위, 담당자, 상태 추적의 정본이다.
- PR은 코드 리뷰와 변경 이력의 정본이다.

설계 변경, 데이터 계약 변경, 출력 포맷 변경은 반드시 문서와 테스트를 함께 갱신한다.

모호하거나 막히면 임의로 확장하지 말고 Linear Issue 또는 GitHub PR 코멘트에 남긴 뒤 Owner 확인을 받는다.
